...
class CsvFileReader():
    ...
    def get_row_col(self, row, col):
        print("get row col -->",self.details[row][col])
...
filepaths = CsvFileReader("D:\Work\Quivers Automation\WoocommerceAutomation\Variable order\OrderSanity.csv")

# commonlocatorsdata_file_path = filepaths.get_row_col(4,1)
# print("common locator -->", commonlocatorsdata_file_path)
...
for i in range(1, locators_files_count):
    ...
    print(f"locater file main path --> {locator_file_main_path}, locater file name {locator_file_name}")
    ...
    print(f"Complete locater file path --> {pagelocatorsdata_file_path}")
